...
**Archon Task ID:** `e9437627-c228-4a38-ae38-270985b572e4`
...
This is the first step in setting up a complete self-hosted media server stack with Real-Debrid integration. The stack includes RDTClient, Radarr, Sonarr, Prowlarr, Bazarr, Jellyfin, and Jellyseerr running in Docker containers on macOS (Apple Silicon M3 Ultra).

Your task is to create the foundational directory structure and base configuration files that all subsequent steps will build upon.

---

## Your Task

Create the complete directory structure and foundational configuration files for the media server stack.

### 1. Create Directory Structure

Create the following directory tree in `/Users/maxwell/Projects/mediamax`:
...
- All config directories should be empty (will be populated by containers on first run)
- Media directories should be empty (will be populated as content is downloaded)
...
```env
# Media Server Stack - Environment Configuration
# Copy this file to .env and configure before running
...
# REQUIRED: You must set this before starting RDTClient
...
# ============================================
# Port Configuration (change if conflicts exist)
# ============================================
RDTCLIENT_PORT=6500
RADARR_PORT=7878
SONARR_PORT=8989
...
**Important Notes:**
- This is a template file - users will copy it to `.env` and customize
- The RD_API_KEY is required for Real-Debrid integration
- Service URLs use container names for internal communication
- Ports can be customized if there are conflicts
...
1. **Directory structure exists:**
   ...
   ```
   Should show all 7 config subdirectories and downloads/movies, downloads/tv

2. **.env.example file created:**
   ```bash
   cat .env.example | grep "RD_API_KEY"
   ```
   Should display the Real-Debrid API key placeholder line

3. **README.md created:**
   ```bash
   wc -l README.md
   ```
   Should show approximately 200+ lines

4. **All files are in correct location:**
   ```bash
   ls -la /Users/maxwell/Projects/mediamax/
   ```
   Should show: config/, media/, .env.example, README.md

---

## Archon Task Update

Once complete, update the Archon task:

```bash
curl -X PUT "http://localhost:8181/api/tasks/e9437627-c228-4a38-ae38-270985b572e4" \
...
- This is step 1 of 6 in the sequential plan
- The next step will create the `docker-compose.yml` file
- Do NOT proceed to other steps - focus only on this task
- Directory structure must be created before Docker containers run
- Config directories will be populated by containers on first startup
